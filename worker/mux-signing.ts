 ...
 * For signed playback with clipping, parameters must be in BOTH the JWT claims AND the URL query string
 */
export function buildSignedPlaybackUrl(
  ...
  if (clips.asset_end_time !== undefined) {
    params.set("end", clips.asset_end_time.toString());
  }
  if (clips.program_start_time !== undefined) {
    params.set("start", clips.program_start_time.toString());
  }
  if (clips.program_end_time !== undefined) {
