 ...
 */
async function enqueueVodJobs(assetId: string, playbackId: string, durationSeconds: number) {
  const jobs = []
  ...
  if (jobs.length > 0) {
    const { error } = await supabase
      ...
      .upsert(jobs, { onConflict: 'source_id,asset_start_seconds,asset_end_seconds', ignoreDuplicates: true })
  ...
  try {
    ...
    if (eventType === 'video.asset.ready') {
      ...
      if (hasLiveStreamId) {
        ...
        console.log(`Asset ${assetId} is from a live stream, skipping webhook enqueue. Worker scheduler will handle segmentation.`)
      } else if (playbackId && duration > 0) {
        // Regular VOD upload (not from a live stream) - enqueue immediately
        console.log(`Enqueuing VOD jobs for regular asset ${assetId} (duration: ${duration}s)`)
        await enqueueVodJobs(assetId, playbackId, duration)
      } else {
        console.warn(`Asset ${assetId} ready but missing playback_id or duration`)
      }
    }
    ...
    if (eventType === 'video.asset.live_stream_completed') {
      ...
      if (!playbackId || duration === 0) {
        console.log(`Asset ${assetId} completed but missing playback_id or duration`)
        return new Response(JSON.stringify({ status: 'success' }), { status: 202, headers: { 'Content-Type': 'application/json' } })
      }
      
      console.log(`Live stream ${assetId} completed with duration ${duration}s - enqueueing all segments`)
      ...
      if (jobs.length > 0) {
        ...
        if (error) {
          console.error(`Error enqueueing segments for completed stream ${assetId}:`, error)
        } else {
          console.log(`Enqueued ${jobs.length} segments for completed stream ${assetId} (${numCompleteWindows} complete windows + ${remainingSeconds > 0 ? 1 : 0} tail)`)
        }
      }
    }
